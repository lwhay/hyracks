...
public abstract class AbstractPropagatePropertiesForUsedVariablesPOperator extends AbstractPhysicalOperator {
    ...
    public void computeDeliveredPropertiesForUsedVariables(ILogicalOperator op, List<LogicalVariable> usedVariables) {
        ...
        while (i < usedVariables.size() && j < downPropsLocal.size()) {
            cols.clear();
            downPropsLocal.get(j).getColumns(cols);
            int k = 0;
            for (; k < cols.size() && i < usedVariables.size(); i++) {
                if (usedVariables.get(i).equals(cols.get(k))) {
                    k++;
                }
            }
            if (k == cols.size()) {
        ...
        for (i = 0; i < j; i++) {
            propsLocal.add(downPropsLocal.get(i));
        }
