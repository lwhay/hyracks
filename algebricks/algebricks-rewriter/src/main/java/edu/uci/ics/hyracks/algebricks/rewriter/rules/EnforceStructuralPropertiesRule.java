...
public class EnforceStructuralPropertiesRule implements IAlgebraicRewriteRule {
	...
	private static final INodeDomain DEFAULT_DOMAIN = new DefaultNodeGroupDomain(
			"__DEFAULT");
	...
	private boolean physOptimizePlan(ILogicalPlan plan,
		...
		for (Mutable<ILogicalOperator> root : plan.getRoots()) {
			...
			AbstractLogicalOperator op = (AbstractLogicalOperator) root
					.getValue();
			op.computeDeliveredPhysicalProperties(context);
			AlgebricksConfig.ALGEBRICKS_LOGGER
					.finest(">>>> Structural properties for "
	...
	private boolean physOptimizeOp(Mutable<ILogicalOperator> opRef,
			IPhysicalPropertiesVector required, boolean nestedPlan,
			IOptimizationContext context) throws AlgebricksException {

		boolean changed = false;
		AbstractLogicalOperator op = (AbstractLogicalOperator) opRef.getValue();
		optimizeUsingConstraintsAndEquivClasses(op);
		PhysicalRequirements pr = op
				.getRequiredPhysicalPropertiesForChildren(required);
		...
		INodeDomain childrenDomain = null;
		{
			...
			for (Mutable<ILogicalOperator> childRef : op.getInputs()) {
				AbstractLogicalOperator child = (AbstractLogicalOperator) childRef
						.getValue();
				// recursive call
				if (physOptimizeOp(childRef, reqdProperties[j], nestedPlan,
						context)) {
					changed = true;
				}
				child.computeDeliveredPhysicalProperties(context);
				IPhysicalPropertiesVector delivered = child
						.getDeliveredPhysicalProperties();
				if (childrenDomain == null) {
					childrenDomain = delivered.getPartitioningProperty()
							.getNodeDomain();
				} else {
					INodeDomain dom2 = delivered.getPartitioningProperty()
							.getNodeDomain();
		...
		for (Mutable<ILogicalOperator> childRef : op.getInputs()) {
			...
			AlgebricksConfig.ALGEBRICKS_LOGGER
					.finest(">>>> Properties delivered by "
							...
							+ "\n");
			IPartitioningRequirementsCoordinator prc = pr
					.getPartitioningCoordinator();
			Pair<Boolean, IPartitioningProperty> pbpp = prc
			...
			IPhysicalPropertiesVector rqd = new StructuralPropertiesVector(
					pbpp.second, reqdProperties[i].getLocalProperties());

			AlgebricksConfig.ALGEBRICKS_LOGGER
					.finest(">>>> Required properties for "
							+ child.getPhysicalOperator() + ": " + rqd + "\n");
			IPhysicalPropertiesVector diff = delivered
					.getUnsatisfiedPropertiesFrom(rqd,
							mayExpandPartitioningProperties,
							context.getEquivalenceClassMap(child),
							context.getFDList(child));
			...
			if (diff != null) {
				...
				AbstractLogicalOperator newChild = ((AbstractLogicalOperator) op
						.getInputs().get(i).getValue());

				if (newChild != child) {
					delivered = newChild.getDeliveredPhysicalProperties();
					IPhysicalPropertiesVector newDiff = newPropertiesDiff(
							newChild, rqd, mayExpandPartitioningProperties,
							context);
					AlgebricksConfig.ALGEBRICKS_LOGGER
							.finest(">>>> New properties diff: " + newDiff
									+ "\n");
		...
		if (opIsRedundantSort) {
			...
			changed = true;
			AbstractLogicalOperator nextOp = (AbstractLogicalOperator) op
					.getInputs().get(0).getValue();
			if (nextOp.getOperatorTag() == LogicalOperatorTag.PROJECT) {
				nextOp = (AbstractLogicalOperator) nextOp.getInputs().get(0)
						.getValue();
			}
			...
			if (transferTo.getOperatorTag() == LogicalOperatorTag.EXCHANGE) {
				...
				transferTo = (AbstractLogicalOperator) transferTo.getInputs()
						.get(0).getValue();
			}
	...
	private IPhysicalPropertiesVector newPropertiesDiff(
			AbstractLogicalOperator newChild,
			IPhysicalPropertiesVector required,
			boolean mayExpandPartitioningProperties,
			IOptimizationContext context) throws AlgebricksException {
		IPhysicalPropertiesVector newDelivered = newChild
				.getDeliveredPhysicalProperties();

		Map<LogicalVariable, EquivalenceClass> newChildEqClasses = context
				.getEquivalenceClassMap(newChild);
		...
		AlgebricksConfig.ALGEBRICKS_LOGGER
				.finest(">>>> Required properties for new op. "
						+ newChild.getPhysicalOperator() + ": " + required
						+ "\n");

		return newDelivered
				.getUnsatisfiedPropertiesFrom(required,
						mayExpandPartitioningProperties, newChildEqClasses,
						newChildFDs);
	}

	private void optimizeUsingConstraintsAndEquivClasses(
	...
	private List<OrderColumn> getOrderColumnsFromGroupingProperties(
			List<ILocalStructuralProperty> reqd,
		...
		for (int j = 0; j <= prefix; j++) {
			...
			returnedProperties.add(new OrderColumn(orderProp.getColumn(),
					orderProp.getOrder()));
		}
	 ...
	 */
	private boolean isRedundantSort(Mutable<ILogicalOperator> opRef,
			IPhysicalPropertiesVector delivered,
			IPhysicalPropertiesVector diffOfProperties,
		...
		AbstractStableSortPOperator sortOp = (AbstractStableSortPOperator) op
				.getPhysicalOperator();
		sortOp.computeLocalProperties(op);
		List<ILocalStructuralProperty> orderProps = sortOp.getOrderProperties();
		return PropertiesUtil.matchLocalProperties(orderProps,
				delivered.getLocalProperties(),
	...
	private void addEnforcers(AbstractLogicalOperator op, int childIndex,
			...
			boolean nestedPlan, IOptimizationContext context)
			throws AlgebricksException {

		IPartitioningProperty pp = diffPropertiesVector
				.getPartitioningProperty();
		if (pp == null
				|| pp.getPartitioningType() == PartitioningType.UNPARTITIONED) {
			addLocalEnforcers(op, childIndex,
					diffPropertiesVector.getLocalProperties(), nestedPlan,
					context);
			IPhysicalPropertiesVector deliveredByNewChild = ((AbstractLogicalOperator) op
					.getInputs().get(0).getValue())
					.getDeliveredPhysicalProperties();
			...
			AbstractLogicalOperator newChild = (AbstractLogicalOperator) op
					.getInputs().get(childIndex).getValue();
			IPhysicalPropertiesVector newDiff = newPropertiesDiff(newChild,
					required, true, context);
			AlgebricksConfig.ALGEBRICKS_LOGGER
					.finest(">>>> New properties diff: " + newDiff + "\n");
			if (newDiff != null) {
				addLocalEnforcers(op, childIndex, newDiff.getLocalProperties(),
						nestedPlan, context);
			}
		}
	}
	...
	private void addLocalEnforcers(AbstractLogicalOperator op, int i,
			List<ILocalStructuralProperty> localProperties, boolean nestedPlan,
			IOptimizationContext context) throws AlgebricksException {
		if (AlgebricksConfig.DEBUG) {
			AlgebricksConfig.ALGEBRICKS_LOGGER
		...
		for (ILocalStructuralProperty prop : localProperties) {
			...
			case LOCAL_GROUPING_PROPERTY: {
				...
				Collection<LogicalVariable> vars = (g
						.getPreferredOrderEnforcer() != null) ? g
		...
		OperatorPropertiesUtil.computeSchemaAndPropertiesRecIfNull(
				(AbstractLogicalOperator) topOp.getValue(), context);
		printOp((AbstractLogicalOperator) topOp.getValue());
	}

	private Mutable<ILogicalOperator> enforceOrderProperties(
			List<LocalOrderProperty> oList, Mutable<ILogicalOperator> topOp,
			boolean isMicroOp, IOptimizationContext context)
			throws AlgebricksException {
		...
		for (LocalOrderProperty o : oList) {
			IOrder ordType = (o.getOrder() == OrderKind.ASC) ? OrderOperator.ASC_ORDER
					: OrderOperator.DESC_ORDER;
			Pair<IOrder, Mutable<ILogicalExpression>> pair = new Pair<IOrder, Mutable<ILogicalExpression>>(
					ordType, new MutableObject<ILogicalExpression>(
							new VariableReferenceExpression(o.getColumn())));
			oe.add(pair);
		}
	...
	private void addPartitioningEnforcers(ILogicalOperator op, int i,
			IPartitioningProperty pp, IPhysicalPropertiesVector required,
			IPhysicalPropertiesVector deliveredByChild, INodeDomain domain,
			IOptimizationContext context) throws AlgebricksException {
		if (pp != null) {
			...
			case UNORDERED_PARTITIONED: {
				...
				List<ILocalStructuralProperty> cldLocals = deliveredByChild
						.getLocalProperties();
				List<ILocalStructuralProperty> reqdLocals = required
						.getLocalProperties();
				boolean propWasSet = false;
				pop = null;
				if (reqdLocals != null && cldLocals != null
						&& allAreOrderProps(cldLocals)) {
					AbstractLogicalOperator c = (AbstractLogicalOperator) op
							.getInputs().get(i).getValue();
					Map<LogicalVariable, EquivalenceClass> ecs = context
							.getEquivalenceClassMap(c);
					List<FunctionalDependency> fds = context.getFDList(c);
					if (PropertiesUtil.matchLocalProperties(reqdLocals,
							cldLocals, ecs, fds)) {
						List<OrderColumn> orderColumns = getOrderColumnsFromGroupingProperties(
								reqdLocals, cldLocals);
						pop = new HashPartitionMergeExchangePOperator(
								orderColumns, varList, domain);
						propWasSet = true;
					}
				}
				if (!propWasSet) {
			...
			case ORDERED_PARTITIONED: {
				pop = new RangePartitionPOperator(
						((OrderedPartitionedProperty) pp).getOrderColumns(),
						domain);
				break;
			}
			case BROADCAST: {
			...
			case RANDOM: {
				...
				/*
				 * if (nd == null) { throw new
				 * AlgebricksException("Unknown node domain for " + rpp); } if
				 * (nd.cardinality() == null) { throw new
				 * AlgebricksException("Unknown cardinality for node domain " +
				 * nd); } if (nd.cardinality() != 1) { throw new
				 * NotImplementedException(
				 * "Random repartitioning is only implemented for target domains of"
				 * + "cardinality equal to 1."); } pop = new
				 * BroadcastPOperator(nd);
				 */
				break;
			}

			default: {
				throw new NotImplementedException("Enforcer for "
						+ pp.getPartitioningType()
