...
//    return status.getJobName();   ///////////// HyracksHadoopCompatLayer
  }
  ...
  public TaskReport[] getTaskReports(TaskType type)  ///////////////////////////////////////////////////////////////////////////////////
    ...
    return ugi.doAs(new PrivilegedExceptionAction<TaskReport[]>() {
      public TaskReport[] run() throws IOException, InterruptedException {
        //HyracksHadoopCompatLayer ////////////
        return new TaskReport[] { new TaskReport( new org.apache.hadoop.mapred.TaskID(rj.getID(), tmpType, 0), 0.0f, State.RUNNING.toString(), 
                null, TIPStatus.RUNNING, 100, 100, new org.apache.hadoop.mapreduce.Counters() )};
        
        /////////////////////////////////////   
//        return cluster.getClient().getTaskReports(getJobID(), tmpType);  //HyracksHadoopCompatLayer ////////////
      }
    });
  }
 ...
 * @throws Exception 
  ...
  public void submit() 
    ...
    if ( compatLayer ){
        // HyracksHadoopCompatLayer ///////////
//        super.conf = new JobConf(Job.this.getConfiguration());
System.out.println("[Job] Number of maps in job configuration: " + super.conf.getNumMapTasks());
        
        String hadoopPath = System.getenv("HADOOP_HOME") != null ? System.getenv("HADOOP_HOME") : System.getenv("HADOOP_PREFIX");
        System.out.println("[Job] hadoopPath: " + hadoopPath);
//        if(hadoopPath != null){
//          Path pathCore = new Path(hadoopPath + "/conf/core-site.xml");
//          super.conf.addResource(pathCore);
//          Path pathMapRed = new Path(hadoopPath + "/conf/mapred-site.xml");
//          super.conf.addResource(pathMapRed);
//          Path pathHDFS = new Path(hadoopPath + "/conf/hdfs-site.xml");
//          super.conf.addResource(pathHDFS);
...
//        }
//        else {
          URL hadoopCore = super.conf.getClass().getClassLoader().getResource("core-site.xml");
          if (hadoopCore != null) {
              super.conf.addResource(hadoopCore);
          }
          URL hadoopMapRed = super.conf.getClass().getClassLoader().getResource("mapred-site.xml");
          if (hadoopMapRed != null) {
              super.conf.addResource(hadoopMapRed);
          }
          URL hadoopHdfs = super.conf.getClass().getClassLoader().getResource("hdfs-site.xml");
          if (hadoopHdfs != null) {
              super.conf.addResource(hadoopHdfs);
          }
System.out.println("[Job] hadoopCore: " + hadoopCore + ", hadoopMapRed: " + hadoopMapRed + ", hadoopHDFS: " + hadoopHdfs);
//        }
        
        hhcl = new HyracksHadoopCompatLayer(super.conf);
        System.out.println("[Job] NewCompatibilityLayer is initialized. Now ready to run!!");
        rj = hhcl.run(super.conf);
 ...
 * @throws Exception 
  ...
  public boolean monitorAndPrintJob() 
      ...
      if( compatLayer ){
           while (!isComplete()) {
               try {
                   int t = Job.getProgressPollInterval(getConfiguration());
                   Thread.sleep(t);
               } catch (InterruptedException ie) {
               }
           }
