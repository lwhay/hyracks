...
public class JobClient extends CLI {
  ...
  //HyracksHadoopCompatLayer ////////////
  private boolean compatLayer = false;
  private HyracksHadoopCompatLayer hhcl = null;
  private JobConf jobConf = null;
  private HyracksHadoopClient hyracksHadoopClient = null;
  ////////////////////////////////////////
   ...
   */
  public JobClient() {
  System.out.println("JobClient() called!!!!");    
  }
  ...
  public JobClient(JobConf conf) throws IOException {
    // HyracksHadoopCompatLayer ////////////
    System.out.println("Hadoop VersionInfo: " + VersionInfo.getVersion());
    ////////////////////////////////////////
    init(conf);
  }
  ...
  public void init(JobConf conf) throws IOException {
      ...
      this.jobConf = conf;
      try {
          this.hyracksHadoopClient = new HyracksHadoopClient();
      } catch (Exception e) {
          // TODO Auto-generated catch block
          e.printStackTrace();
      }
 ...
 * @throws Exception 
   */
  public RunningJob submitJob(String jobFile) throws Exception {
 ...
 * @throws Exception 
   */
  public RunningJob submitJob(final JobConf conf) throws Exception {
  ...
  public RunningJob submitJobInternal(final JobConf conf)
      ...
      if ( compatLayer ){
          // HyracksHadoopCompatLayer ///////////
System.out.println("[JobClient] Number of maps in job configuration: " + conf.getNumMapTasks());
          jobConf = conf;
          
          String hadoopPath = System.getenv("HADOOP_HOME") != null ? System.getenv("HADOOP_HOME") : System.getenv("HADOOP_PREFIX");
          System.out.println("[JobClient] hadoopPath: " + hadoopPath);
          if(hadoopPath != null){
//            Path pathCore = new Path(hadoopPath + "/conf/core-site.xml");
//            jobConf.addResource(pathCore);
//            Path pathMapRed = new Path(hadoopPath + "/conf/mapred-site.xml");
//            jobConf.addResource(pathMapRed);
//            Path pathHDFS = new Path(hadoopPath + "/conf/hdfs-site.xml");
//            jobConf.addResource(pathHDFS);
              Path pathCore = new Path(hadoopPath + "/etc/hadoop/core-site.xml");
              jobConf.addResource(pathCore);
              Path pathMapRed = new Path(hadoopPath + "/etc/hadoop/mapred-site.xml");
              jobConf.addResource(pathMapRed);
              Path pathHDFS = new Path(hadoopPath + "/etc/hadoop/hdfs-site.xml");
              jobConf.addResource(pathHDFS);
          }
          else {
            URL hadoopCore = jobConf.getClass().getClassLoader().getResource("core-site.xml");
            if (hadoopCore != null) {
                jobConf.addResource(hadoopCore);
            }
            URL hadoopMapRed = jobConf.getClass().getClassLoader().getResource("mapred-site.xml");
            if (hadoopMapRed != null) {
                jobConf.addResource(hadoopMapRed);
            }
            URL hadoopHdfs = jobConf.getClass().getClassLoader().getResource("hdfs-site.xml");
            if (hadoopHdfs != null) {
                jobConf.addResource(hadoopHdfs);
            }
System.out.println("[JobClient] hadoopCore: " + hadoopCore + ", hadoopMapRed: " + hadoopMapRed + ", hadoopHDFS: " + hadoopHdfs);
          }
          
          hhcl = new HyracksHadoopCompatLayer(conf);
          System.out.println("[JobClient] NewCompatibilityLayer is initialized. Now ready to run!!");
          return hhcl.run(conf);
          ////////////////////////////////////////
      }
      else {  
        try {
  ...
  public ClusterStatus getClusterStatus() throws IOException {
      ...
      else{ // HyracksHadoopCompatLayer
          int aliveNCs = 0;
          
          Map<String, NodeControllerInfo> ncis = new TreeMap<String, NodeControllerInfo>();
          try {
              ncis.putAll(hyracksHadoopClient.getConnection().getNodeControllerInfos());
          } catch (Exception e) {
            // TODO Auto-generated catch block
              e.printStackTrace();
          }
         
          for (Map.Entry<String, NodeControllerInfo> entry : ncis.entrySet()) {
              if (entry.getValue().getStatus().equals(NodeStatus.ALIVE) )
                  aliveNCs++;
          }
System.out.println("[JobClient][getClusterStatus] number of aliveNCs: " + aliveNCs);    
          return new ClusterStatus(
            aliveNCs,                                                       // hadoop api: taskTrackers.size() - getBlacklistedTrackerCount(),
            0,                                                              // hadoop api: getBlacklistedTrackerCount(),
            10 * 60 * 1000,                                                 // hadoop api: TASKTRACKER_EXPIRY_INTERVAL,
            jobConf.getNumMapTasks(),                                       // hadoop api: totalMaps,
            jobConf.getNumReduceTasks(),                                    // hadoop api: totalReduces,
            jobConf.getInt("mapred.tasktracker.map.tasks.maximum", 1),      // hadoop api: totalMapTaskCapacity,
            jobConf.getInt("mapred.tasktracker.reduce.tasks.maximum", 1),   // hadoop api: totalReduceTaskCapacity, 
            JobTrackerStatus.RUNNING,                                       // hadoop api: state,   
            0,                                                              // hadoop api: getExcludedNodes().size());
            0);                                                             // hadoop api: getGraylistedTrackerCount(),
      }
  }
