...
public class DelimitedDataTupleParserFactory implements ITupleParserFactory {
    ...
    public ITupleParser createTupleWithAutoGeneratedPKParser(final IHyracksTaskContext ctx) {
        return new ITupleParser() {
            ...
            public void parse(InputStream in, IFrameWriter writer) throws HyracksDataException {
                try {
                    ...
                    while (cursor.nextRecord()) {
                    	...
                    	long uuidMSB = 0;
                    	long uuidLSB = 0;
                    	createRandomUUID(uuidMSB, uuidLSB);
                    	
                    	tb.reset();
                        for (int i = 0; i < valueParsers.length; ++i) {
                            if (!cursor.nextField() && primaryKeyPosition < i) {
                                break;
                            } else if (primaryKeyPosition > i) {
                            	Integer64SerializerDeserializer.INSTANCE.serialize(uuidMSB, dos);
                            	Integer64SerializerDeserializer.INSTANCE.serialize(uuidLSB, dos);
                                tb.addFieldEndOffset();
                                break;
                            } else if (primaryKeyPosition == i) {
                            	Integer64SerializerDeserializer.INSTANCE.serialize(uuidMSB, dos);
            ...
            protected void createRandomUUID(long msb, long lsb) {
            ...
            protected void uuidBitsFromBytes(long[] bits, byte[] randomBytes) {
                bits[0] = 0;
                bits[1] = 0;
