...
public abstract class IMRUJobV3<Model extends Serializable, Data extends Serializable, IntermediateResult extends Serializable>
    ...
    @Override
    final public void parse(IHyracksTaskContext ctx, InputStream input,
            TupleWriter output) throws IOException {
        parse(ctx, input, new DataWriter<Data>(output));
    }

    public abstract void parse(IHyracksTaskContext ctx, InputStream input,
            DataWriter<Data> output) throws IOException;

    @Override
    final public IntermediateResult map(final IHyracksTaskContext ctx,
            final TupleReader input, Model model, int cachedDataFrameSize)
            throws IOException {
    ...
    public abstract IntermediateResult map(IHyracksTaskContext ctx,
            Iterator<Data> input, Model model) throws IOException;

    @Override
    public IntermediateResult reduce(IHyracksTaskContext ctx,
            Iterator<IntermediateResult> input) throws HyracksDataException {
    ...
    @Override
    public void update(IHyracksTaskContext ctx,
