...
public class OrderedSlotManager implements ISlotManager {
	...
	protected IBTreeFrame frame;

	@Override
	public int findTupleIndex(ITupleReference searchKey,
			IBTreeTupleReference frameTuple, MultiComparator multiCmp,
	...
	public int insertSlot(int tupleIndex, int tupleOff) {
		if (tupleIndex < 0) {
			...
			System.arraycopy(frame.getBuffer().array(), slotEndOff, frame
					.getBuffer().array(), slotEndOff - slotSize, length);
			setSlot(slotOff, tupleOff);
			return slotOff;
		}
	}
